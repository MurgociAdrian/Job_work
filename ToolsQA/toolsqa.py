...
class TextBoxPage(ParentClassPOM):
    ...
    def write_in_textbox(self, text_box_name, text_to_write, entry_type="new"):

        html_tag = "BadTag!"

        if text_box_name == TextBoxPage.LIST_AVAILABLE_ID_FOR_TEXTBOX[0] \
           or text_box_name == TextBoxPage.LIST_AVAILABLE_ID_FOR_TEXTBOX[1]:
            html_tag = "input"
        elif text_box_name == TextBoxPage.LIST_AVAILABLE_ID_FOR_TEXTBOX[2] \
              or text_box_name == TextBoxPage.LIST_AVAILABLE_ID_FOR_TEXTBOX[3]:
            html_tag = "textarea"
        else:
            print("Wrong textbox name! Options are: userName, userEmail, currentAddress, permanentAddress")

        try:
            text_box = self.driver_obj.find_element_by_xpath(f"//form[@id='userForm']/div[@id='{text_box_name}-wrapper']/"
                                                             f"div[@class='col-md-9 col-sm-12']/"
                                                             f"{html_tag}[@id='{text_box_name}']")
        except Exception as e:
            print(e)
        else:
            if entry_type == "new":
            ...
            elif entry_type == "continue":
                text_box.send_keys(text_to_write)
            else:
                print("Wrong type of entry! Possible arguments are: new, continue")
    ...
    def check_if_answer_is_shown(self, list_with_textbox_written_text=None):
        ...
        try:
            ...
            print("Submit button didn't show anything. Possible errors:\n"
                  "1. The info in textboxes was never introduced or was introduced wrong.\n"
                  "2. CLICK SUBMIT BUTTON method was never used above.\n"
                  "3. Submit button is not working properly (bug).")
            return e
        else:
...
# j.write_in_textbox("userEmail", "a3fm3@yahoo.com")
# j.write_in_textbox("currentAddress", "ceva")
# j.write_in_textbox("permanentAddress", "Str Medg, Bl h2, ap 26 :)")
# j.click_submit_button()
# print(j.check_if_answer_is_shown())
# j.write_in_textbox("userName", "1111111111111", "continue")
# j.write_in_textbox("permanentAddress", "Etaj 8", "continue")
