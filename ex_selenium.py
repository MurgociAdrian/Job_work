...
class ReleasePage(ParentClassPOM):
    ...
    def return_last_py_version(self):

        return self.driver_obj.find_element_by_xpath("//ol[@class='list-row-container menu']/li"
    ...
    def search_string_in_specific_release_table(self, string_to_search):
...
class TestScripts:
    ...
    def check_coresp_rel_for_latest_py_version(self):
        ...
        release_page = ReleasePage(self.driver)
        last_py_version = release_page.return_last_py_version()
        bool_coresp = release_page.search_string_in_specific_release_table(last_py_version.text)
