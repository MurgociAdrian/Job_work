...
class ReleasePage(ParentClassPOM):
    ...
    def return_specific_first_row_from_py_table(self, column, conversion_type):

        if conversion_type == "date":
            return datetime.strptime(self.driver_obj.find_element_by_xpath(f"//ol[@class='list-row-container menu']/li"
                                                                           f"/span[@class='{column}']").text, "%Y-%m-%d")

        return self.driver_obj.find_element_by_xpath(f"//ol[@class='list-row-container menu']/li"
                                                     f"/span[@class='{column}']").text
    ...
    def return_date_for_specific_version(self, string_last_py_ver):
        ...
        for item in list_comb_rel_nr_date:
            if string_last_py_ver in item[0].text:
                return parser.parse(item[1].text)
